...
class Hour extends Model
{
    ...
    public function scopeProjectsSum($query,$user_id, $week){
     ...
     public function scopeTotalHours($query,$user_id, $week){
        ...
        $hours=DB::select("SELECT SUM(hours.hours) as total  FROM `hours` JOIN `users` ON hours.user_id=users.id WHERE users.id=? AND hours.week=?",[$user_id, $week]);
    ...
    public function scopeWeeksHoursWithoutProjects($query,$user_id, $week){
        ...
        $hours=DB::select("SELECT SUM(hours.hours) as hours, hours.day FROM `hours` JOIN `users` ON hours.user_id=users.id WHERE users.id=? AND hours.week=? AND hours.project_id IS NULL GROUP BY hours.day ASC",[$user_id, $week]);
