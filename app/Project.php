...
//veido attiecību starp projekta un nostrādāto stundu datu bāzes tabulām, kurā projektam ir vairakas nostrādātās stundas ieraksti
   ...
   //Funkcija, kurā no datu bāzes iegūst projektu lietotāja kas ir apstiprināts projekta amatā
    public function scopeAssignedProjects($query,$user_id){
       ...
       $assignprojects=DB::select("SELECT projects.title, projects.start_date, projects.end_date, projects.assign_till, projects.id 
       ...
       JOIN `users` ON user_positions.user_id=users.id WHERE users.id=? AND user_positions.accepted='1' 
       GROUP BY projects.title, projects.start_date ASC, projects.end_date, projects.assign_till, projects.id",[$user_id]);
   ...
   //funkcija kurā iegūst projekta nosaukumu un identifikātoru nostrādāto stundu funkcijai
    public function scopeHourProjects($query,$user_id){
     ...
     public function scopeHourEditProjects($query,$user_id,$project_id){
        ...
        JOIN `user_positions` ON user_positions.position_id=positions.id 
        JOIN `users` ON user_positions.user_id=users.id 
        WHERE users.id=? AND user_positions.accepted=true AND projects.id 
        NOT IN(SELECT projects.id FROM `projects` WHERE projects.id=?) 
        GROUP BY projects.id, projects.title, projects.start_date, projects.end_date",[$user_id, $project_id]);
